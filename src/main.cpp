...
uint256 hashGenesisBlock("0xfe84b5019e67b910ab86bacc9a397c9439e3834ff99c5179012dc48897fb64bb");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FalconCoin: starting difficulty is 1 / 2^12
...
{
    // FalconCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 1 * 60; // FalconCoin: each block
static const int64 nTargetSpacing = 1 * 60; // FalconCoin: 60 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // FalconCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, FalconCoin - DarkGravity, written by Evan Duffield - evan@FalconCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // FalconCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // FalconCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "FalconCoin is born!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1493138276;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 29235052;
        ...
        assert(block.hashMerkleRoot == uint256("0xa7de117f14fd4089015e866e652efd3791151670aa2181bd5baf2d53be3bda5a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // FalconCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FalconCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FalconCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FalconCoinMiner(CWallet *pwallet)
{
    printf("FalconCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("FalconCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running FalconCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FalconCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FalconCoinMiner, pwallet));
}
